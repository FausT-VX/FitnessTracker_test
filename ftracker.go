...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	koef := runningCaloriesMeanSpeedMultiplier * runningCaloriesMeanSpeedShift // коэффициент
	return (koef * meanSpeed(action, duration)) * weight / mInKm * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	v2 := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2) // Квадрат средней скорости в м/с
	height = height / cmInM                                  // Рост преобразованный в метры
	return (walkingCaloriesWeightMultiplier*weight + (v2/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	v := swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift // Скорость с коэффициентом
	return v * swimmingCaloriesWeightMultiplier * weight * duration
}
